...
// Prints all printable characters in glcdfont.h
void printCharset(void)
{
    ...
    {
        tft.drawChar(x, y, c, WHITE, BLACK, 1);

        x += 6;                 // 5 px font + 1 px spacing
        if (x > (SCREEN_WIDTH - 6))
        {
        x = 0;
        y += 8;               // 7 px tall + 1 px spacing
        }
...
void lcdTestPattern2(void)
{
    ...
    {
        tft.fillRect(
            tft.width() * c / 8,   // x start (vertical bands)
            0,                     // y
            tft.width() / 8,       // band width
            tft.height(),          // full height
            colors[c]
        );
    }
}
