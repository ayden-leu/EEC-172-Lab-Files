...
// Define GPIO masks used by this OLED wiring 
#define OLED_DC_MASK     0x10   // PIN_18
#define OLED_RESET_MASK  0x20   // PIN_45
#define OLED_CS_MASK     0x80   // PIN_15 
...
*/
  ...
  // DC = 0 for command
  GPIOPinWrite(GPIOA3_BASE, OLED_DC_MASK, 0);

  // CS active low
  GPIOPinWrite(GPIOA3_BASE, OLED_CS_MASK, 0);
  ...
  // Deselect OLED
  GPIOPinWrite(GPIOA3_BASE, OLED_CS_MASK, OLED_CS_MASK);
...
*/
  ...
  // DC = 1 for data
  GPIOPinWrite(GPIOA3_BASE, OLED_DC_MASK, OLED_DC_MASK);
...
*  high or low.
*/
// Change Reset from GPIO28 to GPIO45 (0x20)

  volatile unsigned long delay;

  GPIOPinWrite(GPIOA3_BASE, OLED_RESET_MASK, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA3_BASE, OLED_RESET_MASK, OLED_RESET_MASK);	// RESET = RESET_HIGH
