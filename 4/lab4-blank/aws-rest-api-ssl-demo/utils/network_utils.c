...
static long WlanConnect() {
    ...
    //debug
    UART_PRINT("SSID being used = [%s]\n\r", SSID_NAME);
    UART_PRINT("SECURITY_TYPE = %d\n\r", SECURITY_TYPE);
    ...
    UART_PRINT("Waiting... status=0x%lx connected=%d ip=%d\n\r",
               g_ulStatus, IS_CONNECTED(g_ulStatus), IS_IP_ACQUIRED(g_ulStatus));
...
//    // debug
    ...
    _u16 ipLen = sizeof(ipV4);
    unsigned char dhcpIsOn = 0;
    long ret = 0;

    UART_PRINT("Waiting for DHCP IP...\n\r");
    int tries;

    for (tries = 0; tries < 60; tries++) {   // ~60 tries
        // Let SimpleLink process internal events
        _SlNonOsMainLoopTask();

        // Ask the device what IP it currently has
        ipLen = sizeof(ipV4);
        ret = sl_NetCfgGet(SL_IPV4_STA_P2P_CL_GET_INFO,
                               &dhcpIsOn,
                               &ipLen,
                               (unsigned char*)&ipV4);
        UART_PRINT("try=%d ret=%ld ip=0x%lx\n\r",
                       tries, ret, (unsigned long)ipV4.ipV4);

        if (ret == 0 && ipV4.ipV4 != 0) {
            UART_PRINT("IP Acquired (polled): %d.%d.%d.%d\n\r",
    ...
    UART_PRINT("ERROR: Timed out waiting for DHCP IP.\n\r");
    return -1;
